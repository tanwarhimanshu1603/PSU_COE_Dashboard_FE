...
const AdminUpdateEmployees = () => {
  ...
  useEffect(() => {
    if (location.state?.employee) {
      const updatedEmployee = Object.keys(location.state.employee).reduce((acc, key) => {
        const value = location.state.employee[key];
        
  
        // Handle null or undefined values appropriately
        if (value === null || value === undefined) {
          acc[key] = Array.isArray(employee[key]) && [];
        } else {
          acc[key] = value;
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/v1/admin/update', {
        method: 'PUT',
        headers: {
      ...
      <div className="container">
        ...
        <form onSubmit={handleSubmit} className="form-grid">
          {[
            ...
            { label: 'Functional Knowledge', name: 'primaryTechSkill', type: 'text', value: employee.primaryTechSkill.join(',') },
            { label: 'Functional Knowledge', name: 'primaryProductSubdomain', type: 'text', value: employee.primaryProductSubdomain.join(',') },
